...
public class XGMessageReceiver extends XGPushBaseReceiver {
    ...
    public void onRegisterResult(Context context, int errorCode, XGPushRegisterResult message) {
        try {
            if (XgFlutterPlugin.Companion.isNotInitialized()) {
                Log.w(TAG, "XgFlutterPlugin.instance has not initialized");
                return;
            }
            if (context == null || message == null) {
                String msg = "TPNS token: null" + " error: context==null||message == null";
                XgFlutterPlugin.Companion.toFlutterMethod(Extras.ON_REGISTERED_DEVICE_TOKEN, msg);
                return;
            }
            if (errorCode == XGPushBaseReceiver.SUCCESS) {
                ...
                if (!TextUtils.isEmpty(token)) {
                    String msg = token;
                    XgFlutterPlugin.Companion.toFlutterMethod(Extras.ON_REGISTERED_DONE, msg);
                }
            } else {
    ...
    public void onUnregisterResult(Context context, int errorCode) {
        try {
            ...
            if (context == null) {
                ...
                XgFlutterPlugin.Companion.toFlutterMethod(Extras.UN_REGISTERED, msg);
                return;
            }
            String text = "";
            if (errorCode == XGPushBaseReceiver.SUCCESS) {
    ...
    public void onTextMessage(Context context, XGPushTextMessage message) {
        try {
            ...
            //交由Flutter自主处理
            XgFlutterPlugin.Companion.toFlutterMethod(Extras.ON_RECEIVE_MESSAGE, para);
        } catch (Throwable e) {
...
//                    Extras.TO_FLUTTER_METHOD_NOTIFACTION_SHOW_RESULT, para);
            ...
            //交由Flutter自主处理
            XgFlutterPlugin.Companion.toFlutterMethod(Extras.XG_PUSH_CLICK_ACTION, para);
        } catch (Throwable e) {
    ...
    //                    Extras.TO_FLUTTER_METHOD_NOTIFACTION_SHOW_RESULT, para);
            ...
            para.put(Extras.NOTIFACTION_ACTION_TYPE, notificationActionType);
            //交由Flutter自主处理
            XgFlutterPlugin.Companion.toFlutterMethod(Extras.ON_RECEIVE_NOTIFICATION_RESPONSE, para);
        } catch (Throwable e) {
